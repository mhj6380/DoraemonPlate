...
export class AuthService {  
  ...
  async existsByUserId(user_id: string): Promise<any> {
    ...
    if (user) {
      throw new ConflictException('이미 존재하는 계정입니다. 다른 정보를 입력해 주세요.');
    }
  ...
  async validateUser(user_id: string, pass: string): Promise<any> {
    ...
    if (!bcrypt.compareSync(pass, user.password)) {
      throw new UnauthorizedException("이메일 또는 비밀번호를 다시 확인하세요. 이메일 또는 비밀번호를 잘못 입력하셨습니다.");
    }
  ...
  public async login(user: LoginUserDTO): Promise<any | { status: number }>{
    return this.validateUser(user.user_id, user.password).then(async (userData) => { 
      if(!userData){  
  ...
  async update(user_id:string, updateData:UpdateUserDTO) {
    const user = await this.getOne(user_id);
    await this.userRepository.update({ user_id: updateData.user_id }, updateData);
  }
